...
WATCH_DIR = Path("./chunks")
MD5_FILE = Path("files.md5")
REMOTE_PATH = "telia_privat:plugins"
...
def verify_remote_copy(local_path):
    ...
    # Create temp MD5 file with only the relevant entry
    md5sum = calculate_md5(local_path)  # Ensure correct MD5 sum
    with temp_md5_file.open('w') as dst:
        dst.write(f"{md5sum}  {local_path.name}\n")

    # Run rclone check for this specific file
    result = subprocess.run(
        ["rclone", "check", "--one-way","-C", "md5", str(temp_md5_file), REMOTE_PATH, "-vv"],
        capture_output=True, text=True
    )

    # Clean up temp MD5 file
...
def process_file():
    while True:
        ...
        if is_file_stable(file_path):
            md5sum = calculate_md5(file_path)  # Compute MD5 before upload

            print(f"Starting rclone copy for: {file_path}")
            copy_result = subprocess.run(
                ["rclone", "copy", str(file_path), REMOTE_PATH], capture_output=True, text=True
            )
            ...
            if copy_result.returncode == 0 and verify_remote_copy(file_path):
                # Only append MD5 after successful upload & verification
